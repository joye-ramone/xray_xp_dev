...
#define PPP(a) (100.f*float(a)/float(EngineTOTAL.result))
		...
		F.OutNext	("uSheduler_L: %5.1fms",fShedulerLoad);
		...
		F.OutNext	("Physics:     %5.1fms, %2.1f%%",Physics.result,		PPP(Physics.result));	
		F.OutNext	("  collider:  %5.1fms", ph_collision.result);	
		F.OutNext	("  solver:    %5.1fms, %d",ph_core.result,ph_core.count);	
		F.OutNext	("aiThink:     %5.1fms, %d",AI_Think.result,AI_Think.count);	
		F.OutNext	("  aiRange:   %5.1fms, %d",AI_Range.result,AI_Range.count);
		F.OutNext	("  aiPath:    %5.1fms, %d",AI_Path.result,AI_Path.count);
		F.OutNext	("  aiNode:    %5.1fms, %d",AI_Node.result,AI_Node.count);
		F.OutNext	("aiVision:    %5.1fms, %d",AI_Vis.result,AI_Vis.count);
		F.OutNext	("  Query:     %5.1fms",	AI_Vis_Query.result);
		F.OutNext	("  RayCast:   %5.1fms",	AI_Vis_RayTests.result);
		F.OutSkip	();
...
#define PPP(a) (100.f*float(a)/float(RenderTOTAL.result))
		...
		F.OutNext	("  DT_Render: %5.1fms",RenderDUMP_DT_Render.result);	
		F.OutNext	("  DT_Cache:  %5.1fms",RenderDUMP_DT_Cache.result);	
		F.OutNext	("  Wallmarks: %5.1fms, %d/%d - %d",RenderDUMP_WM.result,RenderDUMP_WMS_Count,RenderDUMP_WMD_Count,RenderDUMP_WMT_Count);
		F.OutNext	("  Glows:     %5.1fms",RenderDUMP_Glows.result);	
		F.OutNext	("  Lights:    %5.1fms, %d",RenderDUMP_Lights.result,RenderDUMP_Lights.count);
		F.OutNext	("  RT:        %5.1fms, %d",RenderDUMP_RT.result,RenderDUMP_RT.count);
		F.OutNext	("  HUD:       %5.1fms",RenderDUMP_HUD.result);	
		F.OutNext	("  P_calc:    %5.1fms",RenderDUMP_Pcalc.result);
		F.OutNext	("  S_calc:    %5.1fms",RenderDUMP_Scalc.result);
		F.OutNext	("  S_render:  %5.1fms, %d",RenderDUMP_Srender.result,RenderDUMP_Srender.count);
		F.OutSkip	();
		F.OutNext	("*** SOUND:   %5.1fms",Sound.result);
		...
		F.OutSkip	();
		F.OutNext	("Input:       %5.1fms",Input.result);
		F.OutNext	("clRAY:       %5.1fms, %d, %2.0fK",clRAY.result,		clRAY.count,r_ps);
		F.OutNext	("clBOX:       %5.1fms, %d, %2.0fK",clBOX.result,		clBOX.count,b_ps);
		F.OutNext	("clFRUSTUM:   %5.1fms, %d",		clFRUSTUM.result,	clFRUSTUM.count	);
		F.OutSkip	();
		F.OutNext	("netClientRecv:   %5.1fms, %d",	netClient1.result,netClient1.count);
		F.OutNext	("netClientSend:   %5.1fms, %d",	netClient2.result,netClient2.count);
		F.OutNext	("netServer:   %5.1fms, %d",		netServer.result,netServer.count);
		F.OutSkip	();

		F.OutSkip	();
		F.OutNext	("TEST 0:      %5.1fms, %d",TEST0.result,TEST0.count);
