...
protected:
	static	int					vscript_log					(ScriptStorage::ELuaMessageType tLuaMessageType, LPCSTR caFormat, va_list marker);
			...
			bool				do_file						(lua_State *L, LPCSTR	caScriptName, LPCSTR caNameSpaceName);
			void				reinit						();
...
public:
	...
	IC		CScriptThread		*current_thread				() const;
			...
			bool				load_file_into_namespace	(lua_State *L, LPCSTR	caScriptName, LPCSTR caNamespaceName);
			bool				namespace_loaded			(lua_State *L, LPCSTR	caName, bool remove_from_stack = true);
...
DLL_API extern LPCSTR get_lvm_name(lua_State* L);
DLL_API extern void set_lvm_name(lua_State* L, LPCSTR name, bool add_index = true);
